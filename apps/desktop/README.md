...
- The app distinguishes confirmed totals from incomplete coverage. Source status reports a missing folder, denied access, unsupported format, unavailable usage, or partial coverage separately. The latest scan health also marks the current shared-world day incomplete when source files or checkpoints cannot be fully read. An unfinished JSONL line waits for completion; a measured zero remains different from an unknown count. Missing usage is never silently set to zero.
