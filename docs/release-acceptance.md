...
- [x] Rust tests cover creator-timezone day buckets, changed historical revisions, SQLite restart persistence, pause retention, deletion cutoff, and capped retry timing.
- [x] Local Supabase pgTAP tests cover world membership/RLS, invite validity, same-device retries, two-device addition, aggregate summaries, owner transfer, leave, and usage deletion.
...
- [ ] Repeat the hosted end-to-end sharing checks on native Windows, including a two-device same-account case and a two-member world.
